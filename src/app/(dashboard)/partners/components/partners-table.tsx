...
export const columns: ColumnDef<Partner>[] = [
  {
    ...
    cell: ({ row }) => (
      <Image
        ...
        height={40}
        width={40}
        className="rounded-full object-cover border min-h-12 min-w-12"
      />
    ),
    enableSorting: false,
    enableHiding: false,
  ...
  {
    accessorKey: "nameEn",
    header: () => <div className="text-start">Name (EN)</div>,
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <time>{new Date(row.getValue("createdAt")).toLocaleDateString()}</time>
    ),
  },
  {
    id: "actions",
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuSeparator />
            <PartnerDeleteMenuItem partner={partner} />
          </DropdownMenuContent>
        </DropdownMenu>
      );
    },
...
function PartnerDeleteMenuItem({ partner }: { partner: Partner }) {
  const { remove } = usePartner();
